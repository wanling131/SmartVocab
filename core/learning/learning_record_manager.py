...
class LearningRecordManager:
    ...
    def update_mastery_level(
        self, record_id, mastery_level, is_correct=True, question_type="translation"
    ):
        """
        更新掌握程度
        ...
        for record in all_records:
            if record["mastery_level"] < 1.0:  # 未完全掌握
                ...
                if record["mastery_level"] < 0.5:
                    ...
                    urgency = (
                        LEARNING_PARAMS["urgency_base"]
                        + days_since_review * LEARNING_PARAMS["urgency_multiplier"]
                    )
                else:
                    # 掌握程度较高，根据时间计算紧急程度
                    days_since_review = (now - last_reviewed).days
