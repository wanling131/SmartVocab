...
class VocabularyLearningManager:
    ...
    def start_learning_session(
        ...
        session_info = {
            ...
            "learning_stage": (
                "choice" if question_type in ["mixed", "choice"] else "translation"
            ),  # 根据题型设置初始阶段
        ...
        return {"success": True, "message": "学习会话开始", "session_info": session_info}
    ...
    def get_active_session(self, user_id, session_type="learning"):
        ...
        if session_record:
            ...
            return {"success": True, "message": "找到活跃会话", "session_info": session_info}
        else:
    ...
    def submit_answer(
        ...
        try:
            ...
            # 获取或创建学习记录
            existing_record = self.learning_record_manager.get_word_learning_record(
                user_id, word_id
            )
            ...
            if (
                ...
                and session_info.get("question_type") == "mixed"
            ):
                ...
                if session_id:
                    success = self.learning_sessions_crud.update(
    ...
    def generate_test_questions(
        self, user_id, word_count=LEARNING_PARAMS["default_test_word_count"], question_types=None
    ):
        """
        生成测试题目
    ...
    def start_review_session(self, user_id, word_count=LEARNING_PARAMS["default_word_count"]):
        ...
        try:
            ...
            if not review_words:
                ...
                all_records = self.learning_record_manager.get_user_learning_records(
                    user_id, limit=1000
                )
            ...
            if not word_details:
                return {"success": False, "message": "暂无需要复习的单词"}
            ...
            return {"success": True, "message": "复习会话开始", "session_info": session_info}

        except Exception as e:
            logger.exception("开始复习会话失败: %s", e)
            return {"success": False, "message": f"开始复习会话失败: {str(e)}"}
...
def main():
    ...
    if session_result["success"]:
        ...
        if current_word:
            logger.info(
                "当前单词: %s - %s", current_word.get("word"), current_word.get("translation")
            )
