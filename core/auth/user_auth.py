...
class UserAuth:
    ...
    def register(self, username, password, email=None):
        ...
        Returns:
            dict: 注册结果
        """
        logger.debug("UserAuth.register: username=%s, email=%s", username, email)

        # 检查用户名是否合法
        if not self._is_valid_username(username):
            logger.debug("UserAuth.register: 用户名不合法 - %s", username)
            return {"success": False, "message": "用户名不合法"}

        # 检查密码是否合法
        if not self._is_valid_password(password):
            logger.debug("UserAuth.register: 密码不合法")
            return {"success": False, "message": "密码不合法"}

        # 检查用户名是否已存在
        if self._check_user_exists(username):
            logger.debug("UserAuth.register: 用户名已存在 - %s", username)
        ...
        if user_id:
            logger.debug("UserAuth.register: 注册成功，用户ID=%s", user_id)
            return {"success": True, "message": "注册成功", "user_id": user_id}
        else:
            logger.warning("UserAuth.register: 创建用户失败")
    ...
    def login(self, username, password):
        ...
        Returns:
            dict: 登录结果
        """
        logger.debug("UserAuth.login: username=%s", username)

        # 查找用户
        users = self.users_crud.search(username, "username")
        if not users:
            logger.debug("UserAuth.login: 用户不存在 - %s", username)
            return {"success": False, "message": "用户不存在"}

        user = users[0]
        logger.debug("UserAuth.login: 找到用户，ID=%s", user["id"])

        # 验证密码
        if self._verify_password(password, user["password_hash"]):
            logger.debug("UserAuth.login: 密码验证成功")
            self.users_crud.update(user["id"], last_login_at=datetime.now())
            return {"success": True, "message": "登录成功", "user_id": user["id"]}
        else:
            logger.debug("UserAuth.login: 密码错误")
    ...
    def change_password(self, user_id, old_password, new_password):
        ...
        return {
            ...
            "message": "密码修改成功" if affected_rows > 0 else "密码修改失败",
        }
    ...
    def _is_valid_username(self, username):
        ...
        检查用户名是否合法（开发阶段放宽限制）
        """
        logger.debug("UserAuth._is_valid_username: 检查用户名 '%s'", username)
        result = (
            username and isinstance(username, str) and 2 <= len(username) <= 50
        )  # 放宽长度限制，移除字符限制
        logger.debug("UserAuth._is_valid_username: 结果=%s", result)
        return result
    ...
    def _check_user_exists(self, username):
        ...
        检查用户是否存在
        """
        logger.debug("UserAuth._check_user_exists: 检查用户是否存在 - %s", username)
        result = len(self.users_crud.search(username, "username")) > 0
        logger.debug("UserAuth._check_user_exists: 结果=%s", result)
        return result
...
def main():
    ...
    # 测试用户注册
    logger.info("1. 测试用户注册:")
    register_result = auth.register("testuser1", "password123", "test1@example.com")
    logger.info("注册结果: %s", register_result)

    # 测试用户登录
    logger.info("2. 测试用户登录:")
    login_result = auth.login("testuser1", "password123")
    logger.info("登录结果: %s", login_result)

    # 测试获取用户信息
    logger.info("3. 测试获取用户信息:")
