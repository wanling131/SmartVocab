...
# 数据库密码：生产环境必须设置，开发环境允许默认值
db_password = os.getenv("DB_PASSWORD")
if not db_password:
    if APP_CONFIG.get("production"):
        raise RuntimeError("生产环境必须设置 DB_PASSWORD 环境变量")
    # 开发/测试环境使用默认密码
    db_password = "test_password"  # 仅用于开发环境
    logger.warning("DB_PASSWORD 未设置，使用开发环境默认密码")
...
class DatabaseManager:
    ...
    def _initialize_pool(self):
        """初始化连接池"""
        try:
            self._pool = pooling.MySQLConnectionPool(**DATABASE_CONFIG)
