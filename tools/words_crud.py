...
class WordsCRUD(BaseCRUD):
    ...
    def get_by_difficulty(
        self, difficulty_level: int, limit: Optional[int] = None, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
        根据难度等级获取单词（带缓存）
        ...
        self.log_operation(
            "根据难度获取单词", difficulty_level=difficulty_level, limit=limit, offset=offset
        )
    ...
    def get_by_dataset_type(
        self, dataset_type: str, limit: Optional[int] = None, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
        根据词库类型获取单词（带缓存）
        ...
        self.log_operation(
            "根据词库类型获取单词", dataset_type=dataset_type, limit=limit, offset=offset
        )
    ...
    def get_by_dataset_and_difficulty(
        self, dataset_type: str, difficulty_level: int, limit: Optional[int] = None, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
        根据词库类型和难度等级获取单词
    ...
    def get_random_words(
        self, exclude_ids: List[int] = None, limit: int = 100, difficulty_range: tuple = (1, 6)
    ) -> List[Dict[str, Any]]:
        """
        获取随机单词（用于生成错误选项等）
