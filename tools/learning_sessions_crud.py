...
class LearningSessionsCRUD(BaseCRUD):
    ...
    def create(self, user_id, session_data, session_type="learning"):
        ...
        """
        logger.debug(
            "LearningSessionsCRUD.create: user_id=%s, session_type=%s", user_id, session_type
        )
    ...
    def get_active_session(self, user_id, session_type="learning"):
        ...
        """
        logger.debug(
            "LearningSessionsCRUD.get_active_session: user_id=%s, session_type=%s",
            user_id,
            session_type,
        )
        ...
        if result:
            ...
            try:
                ...
                logger.debug(
                    "LearningSessionsCRUD.get_active_session: 找到活跃会话，ID=%s", result["id"]
                )
    ...
    def get_by_user(self, user_id, limit=None, offset=0):
        ...
        """
        logger.debug(
            "LearningSessionsCRUD.get_by_user: user_id=%s, limit=%s, offset=%s",
            user_id,
            limit,
            offset,
        )
